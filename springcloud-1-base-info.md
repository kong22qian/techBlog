...
- ==微服务是分布式的解决方案，springcloud是微服务的一种实现==
...
- 考虑到API接口的分类,可以将API接口分为:开放API接口和内网API接口，内网API接口用于局域网，为内部服务器提供服务。开放API接口用于对外部合作单位提供接口调用，需要遵循Oauth2.0权限认证协议。同时还需要考虑安全性、幂等性等问题。
